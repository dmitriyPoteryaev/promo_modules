export const sortModifications = (arrayModificatons, obj, ArrayWithAllOrderPositionStore) => {
  ...
  newArray = newArray.map((modification) => {
    if (
      ArrayWithAllOrderPositionStore.map((orderedPosition) => orderedPosition.certain_name).some(
        (certanName) => modification.certain_name === certanName
      )
    ) {
  ...
  if (
    ...
    !Object.values(obj).some((specificKey: any) => specificKey.checked)
  ) {
    return newArray;
  }
